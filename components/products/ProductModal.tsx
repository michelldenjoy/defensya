...
export default function ProductModal({
  ...
  const images = Array.isArray(producto.imagen)
    ? producto.imagen
    : [producto.imagen];
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/2">
              <div className="flex items-center gap-4">
                <span className="hidden mt-5 md:block text-[10px] font-mono text-gray-400 uppercase tracking-tighter">
                  Sector: {producto.categoria}
                </span>
              </div>
              <button
                onClick={onClose}
                className="py-2 mt-5 hover:bg-defensya-blue/10 hover:text-defensya-blue transition-colors text-gray-400"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className="grid lg:grid-cols-2">
              ...
              <div className="relative aspect-video lg:aspect-auto bg-black overflow-hidden border-r border-white/5">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="relative w-full h-full min-h-100"
                  >
                    <Image
              ...
              {/* ── FICHA TÉCNICA object-cover (Lado Derecho) ── */}
                  ...
                  <div>
                    ...
                    <ul className="space-y-3">
                      {producto.detalles.map((detalle, idx) => (
                        <li
                          key={idx}
                          className="flex gap-4 p-3 bg-gray-50 dark:bg-white/3 border border-gray-100 dark:border-white/5 group hover:border-defensya-blue/20 transition-colors"
                        >
                          <span className="font-mono text-[10px] text-defensya-blue mt-1">
                            [{pad(idx + 1)}]
                          </span>
