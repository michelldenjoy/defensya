...
function ArrowUpRight({ size = 12 }: { size?: number }) {
  return (
    <svg width={size} height={size} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="shrink-0">
...
function DownloadIcon({ size = 12 }: { size?: number }) {
  return (
    <svg width={size} height={size} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="shrink-0">
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1" />
    </svg>
  );
}
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-[1fr_1fr] border border-gray-200 dark:border-white/[0.07]">
            ...
            <div className="flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-white/[0.07] p-8 lg:p-12">
              <div className="flex items-start gap-6 mb-10">
                <div className="relative w-20 h-20 shrink-0 border border-gray-200 dark:border-white/1 p-3">
                  <Image src="/images/ISO9001.png" alt="ISO 9001:2015" fill className="object-contain p-2" />
                </div>
                <div className="pt-1">
              ...
              <div className="border-t border-gray-200 dark:border-white/[0.07]">
                {[
                  ...
                  { label: "Alcance", value: "Ingeniería electrónica y telecomunicación" },
                  { label: "Estado", value: "Certificado vigente" },
                  { label: "Auditoría", value: "Organismo externo independiente" },
                ].map(({ label, value }) => (
                  <div key={label} className="flex items-center justify-between py-3 border-b border-gray-200 dark:border-white/6">
                    <span className="text-[13px] tracking-widest text-gray-400 dark:text-gray-500 uppercase">{label}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white text-right max-w-[55%]">{value}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-col bg-defensya-navy p-8 lg:p-12">
              ...
              <div className="flex flex-col gap-0 border-t border-white/8 flex-1">
                {[
                  { num: "01", label: "Trazabilidad total", desc: "Registro y control de cada proceso productivo, desde el diseño hasta la entrega al cliente." },
                  { num: "02", label: "Auditorías periódicas", desc: "Revisiones internas y externas programadas que verifican el cumplimiento del sistema." },
                  { num: "03", label: "Control de no conformidades", desc: "Detección, análisis y corrección sistemática de desviaciones en productos y procesos." },
                  { num: "04", label: "Validación de proveedores", desc: "Evaluación y seguimiento continuo de la cadena de suministro bajo criterios de calidad certificados." },
                ].map(({ num, label, desc }) => (
                  <div key={num} className="flex gap-5 py-4 border-b border-white/6 group hover:bg-white/3 transition-colors -mx-2 px-2">
                    <span className="font-mono text-[12px] text-defensya-steel tracking-widest shrink-0 pt-0.5 w-6">{num}</span>
                    <div>
                      <p className="text-md font-semibold text-white mb-0.5 uppercase tracking-wide">{label}</p>
      ...
      <div className="relative overflow-hidden h-[420px] w-full">
        ...
        <div className="absolute inset-0" style={{ backgroundColor: "rgba(11, 31, 56, 0.82)" }} />
        ...
        <div className="relative z-10 h-full px-5 sm:px-8 lg:px-16 py-12 sm:py-16 max-w-7xl mx-auto flex flex-col justify-between">
          <p style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: "11px", letterSpacing: "0.28em", textTransform: "uppercase", color: "white" }}>
            Nuestro Compromiso
          </p>
          ...
          <div>
            ...
            <p className="text-sm sm:text-base lg:text-lg text-gray-300 leading-relaxed font-light max-w-5xl">
              ...
              <span className="text-white font-medium">
                entregamos la fiabilidad operativa necesaria para que organizaciones globales operen en entornos críticos.
              </span>{" "}
              En{" "}
              ...
              , garantizando la seguridad en el futuro de la exploración y la defensa global.
            </p>
            <div className="flex items-center gap-4 mt-6">
              <div className="h-px w-10 bg-defensya-blue/50" />
              <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: "9px", letterSpacing: "0.28em", textTransform: "uppercase", color: "rgb(107 114 128)" }}>
